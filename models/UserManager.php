...
class UserManager extends Config
{
    ...
    {
        $db = $this->dbConnect();
        $Ucomment = $db->prepare('SELECT id, post_id, User_pseudo, Notes, contenu, signaler, DATE_FORMAT(comment_date, \'%d/%M/%Y\') AS comment_date_fr FROM Commentaires WHERE User_pseudo = ?');
